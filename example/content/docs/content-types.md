...
Images and other files can be added using normal markdown syntax. Local media files go in the `media` folder inside your content directory.
...
Marmite copies your `media` folder to the output site.

#### Slug-based subfolders

Media can be organized in subfolders named after the content's slug. Marmite auto-discovers `banner.{ext}` and `card.{ext}` files:
...
content/
  media/
    my-post/
      ...
      diagram.png      # Referenced via @/ in markdown
  2024-01-15-my-post.md
```

#### `@/` shorthand

Use `@/` to reference files in the content's own media subfolder without typing the full path:
