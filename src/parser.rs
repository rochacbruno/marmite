...
pub fn get_links_to(html: &str) -> Option<Vec<String>> {
    ...
    let re = Regex::new(re::HREF_HTML_LINKS).expect("Links regex should compile");
...
pub fn get_table_of_contents_from_html(html: &str) -> String {
    let re =
        Regex::new(re::HTML_HEADINGS_WITH_ANCHORS).expect("Table of contents regex should compile");
...
pub fn fix_internal_links(html: &str) -> String {
    let re = Regex::new(re::HTML_ANCHOR_TAGS).expect("Fix internal links regex should compile");
