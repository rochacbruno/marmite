...
#[test]
fn test_subfolder_without_frontmatter_yaml_not_rendered() {
    let temp_dir = TempDir::new().unwrap();
    let input_dir = temp_dir.path().join("input");
    let output_dir = temp_dir.path().join("output");

    fs::create_dir_all(input_dir.join("content").join("random")).unwrap();
    fs::write(input_dir.join("marmite.yaml"), "name: Blog").unwrap();

    fs::write(
        input_dir.join("content").join("random").join("orphan.md"),
    ...
    assert!(
        !output_dir.join("orphan.html").exists(),
        "file in subfolder without frontmatter.yaml should not be rendered"
    );
}

#[test]
fn test_pages_subfolder_without_frontmatter_yaml() {
    let temp_dir = TempDir::new().unwrap();
    let input_dir = temp_dir.path().join("input");
    let output_dir = temp_dir.path().join("output");

    fs::create_dir_all(input_dir.join("content").join("pages")).unwrap();
    fs::write(input_dir.join("marmite.yaml"), "name: Blog").unwrap();

    fs::write(
        input_dir.join("content").join("pages").join("about.md"),
        "---\ntitle: About\n---\n# About\n\nAbout page.\n",
    )
    .unwrap();
    ...
    assert!(
        ...
        "pages subfolder should render content without frontmatter.yaml"
    );
}
